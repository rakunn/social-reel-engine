...
Run `status` after ingest and inspect `intake.requirements` before expensive media work. Consolidate `ask-user` items with any known facts still missing from the prompt; perform `configure` items from verified evidence yourself. LUT binaries are user-supplied: a catalog entry is not proof of an installed file. Request a missing required normalizer, copy it using typed `ingest`, and record verified metadata. If a LUT is already supplied, resolve only its missing semantics instead of asking for the file again. Creative LUTs are optional. Read [inputs.md](references/inputs.md) for the intake and storage rules.
...
Require an explicit user rights statement. When `status` reports `intake.rights.requiresExplicitConfirmation`, present its resolved used-asset inventory and ask the user to confirm permission to use all listed footage, music, LUTs, fonts, captions, and other used assets; consolidate this with remaining factual blockers unless an existing explicit statement already covers the set. If rights are `indeterminate`, finish the reported input/configuration work and author a valid edit before asking; do not request confirmation of an empty or provisional inventory. Merely supplying files, requesting a reel, or having license metadata is not confirmation. Once the user's response covers the exact current used set, run `confirm-rights`; never write rights fields manually. Before long-running media work, ask once for all currently knowable missing profile, transform, rights, and material editorial facts. If footage profile or transform semantics remain ambiguous, declare the job proxy-only, name the exact missing facts and the first blocked command, and proceed only to a watermarked viewing proxy. Do not resolve ambiguity by selecting the LUT whose name seems closest.
