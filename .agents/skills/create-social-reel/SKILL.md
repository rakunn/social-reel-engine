...
Require an explicit user rights statement. Present the asset inventory and ask the user to confirm permission to use all selected footage, music, LUTs, fonts, captions, and other used assets in the output; include this in the consolidated intake request unless an existing explicit statement already covers the set. Merely supplying files, requesting a reel, or having license metadata is not confirmation. Once the edit defines the exact used set and the user's response covers it, run `confirm-rights`; never write rights fields manually. Before long-running media work, ask once for all currently knowable missing profile, transform, rights, and material editorial facts. If footage profile or transform semantics remain ambiguous, declare the job proxy-only, name the exact missing facts and the first blocked command, and proceed only to a watermarked viewing proxy. Do not resolve ambiguity by selecting the LUT whose name seems closest.
