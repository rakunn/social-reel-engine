...
## Captioned carousel derivative forward check

A fresh-context, read-only agent evaluated the same approved clean-carousel-to-captioned-variant request after the skill update. **Pass:** it chose `variant` rather than `new`, kept the clean project untouched, routed copy through per-card `textOverlay`, preserved exact trims/crops/stabilization and per-shot color corrections, and described checksum-gated proxy/rights/color reuse. It required a new captioned rough preview and explicit edit approval, then correctly skipped a redundant color gate only when `status` confirmed the inherited color projection and reviewed-still bytes remained current. It also named every color-relevant invalidator and finished with separate `grade`, `render-carousel`, `qc-carousel`, status, and ready-to-share handoff steps.

## Philippines styled-carousel derivative forward check

A fresh-context, read-only agent evaluated the Bohol styled-derivative scenario after the skill update. **Pass (8/8):** it ran `variant`, inspected `style --list`, applied `philippines-island-editorial`, and immediately ran `analyze`. It preserved the inherited card timing, order, crops, stabilization, corrections, treatments, and LUT choices; routed copy through per-card `textOverlay`; reported the preset and actual display/body/metadata pairing at the new rough gate; classified typography as editorial-only; preserved exact color only when `status` confirmed it; treated all selected role fonts as rights assets; and explicitly rejected generated, inferred, or transliterated Baybayin. It finished with the correct serial `grade`, `render-carousel`, `qc-carousel`, and `status` path.
